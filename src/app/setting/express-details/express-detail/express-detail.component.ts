import { Component, OnInit } from '@angular/core';
import { NgForm } from '@angular/forms';
import { MyErrorStateMatcher } from 'src/app/default.error-matcher';
import { NotificationService } from 'src/app/services/notification.service';
import { SettingService } from 'src/app/services/setting/setting.service';

@Component({
  selector: 'express-detail',
  templateUrl: './express-detail.component.html',
  styleUrls: ['./express-detail.component.css']
})
export class ExpressDetailComponent implements OnInit {
  matcher = new MyErrorStateMatcher();
  
  constructor(public service: SettingService, private toastr: NotificationService) { }

  ngOnInit(): void {
    this.resetForm();
    this.service.getExpressDetails();
    this.service.refreshExpressList();
  }

  resetForm(form?: NgForm)
  {
    if(form != null)
      form.resetForm();
    this.service.expressFormData = {
      Id: 0,
      ExpressName: ''
    }
  }

  onSubmit(form: NgForm) {
    if(this.service.expressFormData.Id ==0)
    {
      this.insertRecord(form);
    }
    else
    {
      this.updateRecord(form);
    }
  }

  insertRecord(form: NgForm) {
    this.service.postExpress().subscribe(
      res => {
        this.resetForm(form);
        this.service.refreshExpressList();
        this.toastr.showSucces("Saved Successfully");
      },
      err => {
        this.toastr.showError("Data can't save");
      }
    )
  }

  updateRecord(form: NgForm) {
    this.service.putExpress().subscribe(
      res => {
        this.resetForm(form);
        this.service.refreshExpressList();
        this.toastr.showInfo("Data Updated");
      },
      err => {
        console.log(err);
        this.toastr.showError("Not updated yet")
      }
    )
   }

}
