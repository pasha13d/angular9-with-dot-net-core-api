...
export class ColorDetailComponent implements OnInit {
  ...
  onSubmit(form: NgForm) {
    if(this.service.colorFormData.Id ==0)
    {
    ...
    else
    {
      ...
      console.log(this.service.colorFormData);
    }
  }

  insertRecord(form: NgForm) {
    this.service.postColor().subscribe(
      res => {
        this.resetForm(form);
        this.service.refreshColorList();
  ...
  updateRecord(form: NgForm) {
    this.service.putColor().subscribe(
      res => {
        this.resetForm(form);
        this.service.refreshColorList();
